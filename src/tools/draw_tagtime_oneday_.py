...
def save_figure(tags, output):
    ...
    for tagd in sorted(tags.items(), key = lambda x: x[0]): # ex) tagd[0] : 20010115
        while True:
            ...
            if tagd[0] > now:
                x.append(now)
                for kt in kinds_tag:
                    for peer in data[kt].keys():
                        if peer in remove_peer:
                            data[kt][peer].append("-")
                        else:
                            data[kt][peer].append(0)
            else:
                break
        x.append(tagd[0])
    ...
    if way == "tag_name":
        ...
        ax.set_title(y_axis)
        output = output + "_" + args.tag_name + ".png"
        ...
        for d in data.items():
            ...
            if d[0] == "prepending_remove":
                ...
                for t in d[0].split("_"):
                    ta += t[0].upper()
            mean, median, std = allrolling_std.allrolling_std(data[d[0]].values(), windows)
            results[ta] = dict()
...
output = "/home/tktbtk/Data/graphs/paper"+file_name
